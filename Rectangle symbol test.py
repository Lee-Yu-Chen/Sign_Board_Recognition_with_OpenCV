        ...
        if len(yup)!=0 and len(ydown)!=0 and len(xleft)!=0 and len(xright)!=0:
            ...
            if des1 is None:
                ...
                for i in range(len(li1)):
                    ...
                    '''
                    img1 = cv.drawMatches(maskedgray,kp1,Traffic,kp2,matches[:1000],None,flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
                    img2 = cv.drawMatches(maskedgray,kp1,Shapes,kp3,matches[:1000],None,flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
                    img3 = cv.drawMatches(maskedgray,kp1,Face,kp4,matches[:1000],None,flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
                    img4 = cv.drawMatches(maskedgray,kp1,Measuredis,kp5,matches[:1000],None,flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
